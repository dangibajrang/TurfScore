...
export function LiveScoringScreen({ matchId, match, scoring }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4" data-testid="live-scoring-screen">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <Link to={`/matches/${matchId}`} className="text-sm text-text-muted hover:text-primary">
            ← {match.name}
          </Link>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <div className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            />
            <CurrentBowler innings={inn} scorecardInnings={scInn} playersById={playersById} />
          </div>
          ...
          {!presentation.inningsComplete ? (
            <div className="sticky bottom-2 z-20 rounded-card border border-border-subtle bg-background/95 p-3 shadow-card backdrop-blur md:static md:bg-transparent md:p-0 md:shadow-none md:backdrop-blur-none">
              <ScoringKeypad
                disabled={busy}
                onRuns={submitRuns}
      ...
      <ConfirmDialog
        ...
        description="This rebuilds match state from remaining deliveries. Score will update from the server."
        confirmLabel="Undo"
        danger
